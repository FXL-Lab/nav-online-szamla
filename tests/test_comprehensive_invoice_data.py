...
from nav_online_szamla.models import (
    NavCredentials, InvoiceDirection, QueryInvoiceDigestResponseType,
    InvoiceDigestType, InvoiceDetail, BasicResultType, BasicHeaderType
)
...
class TestComprehensiveInvoiceData:
    ...
    def test_get_all_invoice_data_for_date_range_basic(self, client, credentials, sample_invoice_digest, sample_invoice_detail):
        ...
        with patch.object(client, 'validate_credentials'):
            with patch.object(client, 'query_invoice_digest', return_value=digest_response):
                with patch.object(client, 'query_invoice_data', return_value=sample_invoice_detail):
    ...
    def test_get_all_invoice_data_for_date_range_multiple_pages(self, client, credentials, sample_invoice_digest, sample_invoice_detail):
        ...
        with patch.object(client, 'validate_credentials'):
            with patch.object(client, 'query_invoice_digest', side_effect=mock_query_digest):
                with patch.object(client, 'query_invoice_data', side_effect=mock_query_data):
    ...
    def test_get_all_invoice_data_for_date_range_with_max_limit(self, client, credentials, sample_invoice_digest, sample_invoice_detail):
    ...
    def test_get_all_invoice_data_with_progress_callback(self, client, credentials):
        ...
        def progress_callback(current_count, total_estimated, current_invoice_number):
            progress_calls.append((current_count, total_estimated, current_invoice_number))
    ...
    def test_invalid_date_range(self, client, credentials):
        ...
        with pytest.raises(NavValidationException, match="Start date must be before end date"):
    ...
    def test_date_range_too_large(self, client, credentials):
        ...
        start_date = datetime.now() - timedelta(days=400)  # More than MAX_DATE_RANGE_DAYS
        end_date = datetime.now()
